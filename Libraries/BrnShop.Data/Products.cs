...
namespace BrnShop.Data
{
    ...
    public partial class Products
    {
        ...
        public static ProductInfo BuildProductFromReader(IDataReader reader)
        {
            ...
            productInfo.Weight = TypeHelper.ObjectToDecimal(reader["weight"]);
            productInfo.ShowImg = reader["showimg"].ToString();
            productInfo.Number = TypeHelper.ObjectToInt(reader["number"]);
        ...
        public static PartProductInfo BuildPartProductFromReader(IDataReader reader)
        {
            ...
            partProductInfo.Weight = TypeHelper.ObjectToDecimal(reader["weight"]);
            partProductInfo.ShowImg = reader["showimg"].ToString();
            partProductInfo.Number = TypeHelper.ObjectToInt(reader["number"]);
        ...
        public static ExtProductAttributeInfo BuildExtProductAttributeFromReader(IDataReader reader)
        {
            ...
            extProductAttributeInfo.IsFilter = TypeHelper.ObjectToInt(reader["isfilter"]);
        ...
        {
            return BrnShop.Core.BSPData.RDBS.AdminGetProductList(pageSize, pageNumber, condition);
        }
        ...
        /// <param name="sortDirection">排序方向</param>
        /// <returns></returns>
        public static List<PartProductInfo> GetCategoryProductList(int pageSize, int pageNumber, int brandId, int cateId, int onlyStock, int sortColumn, int sortDirection)
        {
            List<PartProductInfo> partProductList = new List<PartProductInfo>();
            IDataReader reader = BrnShop.Core.BSPData.RDBS.GetCategoryProductList(pageSize, pageNumber, brandId, cateId, onlyStock, sortColumn, sortDirection);
            while (reader.Read())
            {
                PartProductInfo partProductInfo = BuildPartProductFromReader(reader);
                partProductList.Add(partProductInfo);
        ...
        {
            return BrnShop.Core.BSPData.RDBS.GetBrandProductCount(brandId, cateId, onlyStock);
        }
        ...
        {
            BrnShop.Core.BSPData.RDBS.DeleteProductImageById(pImgId);
            if (_productnosql != null)
        ...
        {
            BrnShop.Core.BSPData.RDBS.SetProductMainImage(pid, pImgId);
            if (_productnosql != null)
                _productnosql.ClearProductImage(pid);
        }
        ...
        {
            BrnShop.Core.BSPData.RDBS.ChangeProductImageDisplayOrder(pImgId, displayOrder);
            if (_productnosql != null)
                _productnosql.ClearProductImage(pid);
        }
