...
namespace BrnShop.Web.Controllers
{
    ...
    public partial class OrderController : BaseWebController
    {
        ...
        public ActionResult ConfirmOrder()
        {
            ...
            model.PayMode = 1;// payMode;目前只用在线支付
            ...
            {
                model.ShipFee = ((IShipPlugin)model.DefaultShipPluginInfo.Instance).GetShipFee(model.TotalWeight, (model.ProductAmount - model.FullCut), cartInfo.SelectedOrderProductList, DateTime.Now, model.DefaultFullShipAddressInfo.ProvinceId, model.DefaultFullShipAddressInfo.CityId, model.DefaultFullShipAddressInfo.RegionId, WorkContext.PartUserInfo);
            }

            model.OrderAmount = model.ProductAmount - model.FullCut + model.ShipFee;

            model.CartInfo = cartInfo;
        ...
        public ActionResult SubmitOrder()
        {
            lock (_locker)
            {
                ...
                //验证已经通过,进行订单保存
                OrderInfo orderInfo = Orders.CreateOrder(WorkContext.PartUserInfo, cartInfo.SelectedOrderProductList, singlePromotionList, fullShipAddressInfo, payMode == 0 ? 0 : 1, shipPluginInfo, payCreditCount, couponList, fullCut, buyerRemark, bestTime, WorkContext.IP);
                if (orderInfo != null)
                {
                    int pCount = 0;
                    //删除剩余的满赠赠品
