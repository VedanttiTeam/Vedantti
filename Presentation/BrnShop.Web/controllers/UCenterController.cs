...
namespace BrnShop.Web.Controllers
{
    ...
    public partial class UCenterController : BaseWebController
    {
        ...
        public ActionResult OrderList()
        {
            ...
            PageModel pageModel = new PageModel(10, page, Orders.GetUserOrderCount(WorkContext.Uid, startAddTime, endAddTime, orderState));
        ...
        public ActionResult FavoriteProductList()
        {
            ...
            PageModel pageModel = new PageModel(10, page, productName.Length == 0 ? Favorites.GetFavoriteProductCount(WorkContext.Uid) : Favorites.GetFavoriteProductCount(WorkContext.Uid, productName));

            FavoriteProductListModel model = new FavoriteProductListModel()
            {
                ProductList = productName.Length == 0 ? Favorites.GetFavoriteProductList(pageModel.PageSize, pageModel.PageNumber, WorkContext.Uid) : Favorites.GetFavoriteProductList(pageModel.PageSize, pageModel.PageNumber, WorkContext.Uid, productName),
                PageModel = pageModel,
        ...
        public ActionResult AddToFavorite()
        {
            ...
            //当商品不存在时
            if (Products.GetPartProductById(pid) == null)
        ...
        public ActionResult ReviewProduct()
        {
            ...
            if (orderInfo.OrderState != (int)OrderState.Received)
                return AjaxResult("noreceive", "订单还未收货,不能评价");
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult OrderAfterService()
        {
            int oid = WebHelper.GetQueryInt("oid");
            ...
            if (orderInfo == null || orderInfo.Uid != WorkContext.Uid)
                return PromptView("订单不存在");
            if (orderInfo.OrderState != (int)OrderState.Received)
                return PromptView("订单还没有收货，不能退换货");

            OrderAfterServiceModel model = new OrderAfterServiceModel()
            {
                OrderInfo = orderInfo,
                OrderProductList = Orders.GetOrderProductList(oid),
                OrderAfterServiceList = OrderAfterServices.GetOrderAfterServiceList(oid)
            };
            return View(model);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult ApplyOrderAfterService()
        {
            if (WebHelper.IsGet())
            {
                int oid = WebHelper.GetQueryInt("oid");
                int recordId = WebHelper.GetQueryInt("recordId");
                ...
                ApplyOrderAfterServiceModel model = new ApplyOrderAfterServiceModel()
                {
                    Oid = oid,
                    RecordId = recordId,
            ...
            else
            {
                ...
                string applyReason = WebHelper.GetFormString("applyReason");

                OrderInfo orderInfo = Orders.GetOrderByOid(oid);
                if (orderInfo == null || orderInfo.Uid != WorkContext.Uid)
                    return AjaxResult("noorder", "订单不存在");
                if (orderInfo.OrderState != (int)OrderState.Received)
                    return AjaxResult("noreceive", "订单还没有收货，不能退换货");
                ...
                if (type < 0 || type > 2)
                    return AjaxResult("wrongtype", "请选择类型");

                if (count < 1)
                    return AjaxResult("nocount", "请输入数量");
                if (count > orderProductInfo.SendCount)
                    return AjaxResult("muchcount", "数量最多输入" + orderProductInfo.SendCount);

                if (applyReason.Length == 0)
                    return AjaxResult("noapplyreason", "请输入申请原因");
                if (applyReason.Length > 150)
                    return AjaxResult("muchapplyreason", "申请原因最多输入150个字");

                OrderAfterServices.ApplyOrderAfterService(orderProductInfo, count, type, applyReason);
                return AjaxResult("success", oid.ToString());
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult SendOrderAfterService()
        {
            if (WebHelper.IsGet())
            {
                int asId = WebHelper.GetQueryInt("asId");

                OrderAfterServiceInfo orderAfterServiceInfo = OrderAfterServices.GetOrderAfterServiceByASId(asId);
                if (orderAfterServiceInfo == null || orderAfterServiceInfo.Uid != WorkContext.Uid)
                    return AjaxResult("noorderafterservice", "订单售后服务不存在");
                if (orderAfterServiceInfo.State != (int)OrderAfterServiceState.CheckAgree)
                    return AjaxResult("nocheckagree", "管理员没有同意，不能退换货");

                SendOrderAfterServiceModel model = new SendOrderAfterServiceModel()
                {
                    OrderAfterServiceInfo = orderAfterServiceInfo,
                    ShipCompanyList = ShipCompanies.GetShipCompanyList()
                };
                return View(model);
            }
            else
            {
                int asId = WebHelper.GetQueryInt("asId");
                int shipCoId = WebHelper.GetFormInt("shipCoId");
                string shipSN = WebHelper.GetFormString("shipSN");
                string consignee = WebHelper.GetFormString("consignee");
                string mobile = WebHelper.GetFormString("mobile");
                int regionId = WebHelper.GetFormInt("regionId");
                string address = WebHelper.GetFormString("address");
                ...
                if (orderAfterServiceInfo == null)
                    return AjaxResult("noorderafterservice", "此售后服务不存在");
                if (orderAfterServiceInfo.State != (int)OrderAfterServiceState.CheckAgree)
                    return AjaxResult("wrongstate", "不能进行此操作");

                ShipCompanyInfo shipCompanyInfo = ShipCompanies.GetShipCompanyById(shipCoId);
                if (shipCompanyInfo == null)
                    return AjaxResult("noshipcompany", "配送公司不存在");

                if (shipSN.Length == 0)
                    return AjaxResult("noshipsn", "请输入配送单号");
                if (shipSN.Length > 30)
                ...
                if (consignee.Length == 0)
                    return AjaxResult("noconsignee", "请输入收货人");
                else if (consignee.Length > 10)
                    return AjaxResult("muchconsignee", "收货人不能超过10个字");

                if (mobile.Length == 0)
                    return AjaxResult("nomobile", "请输入手机号");
                else if (!ValidateHelper.IsMobile(mobile))
                    return AjaxResult("wrongmobile", "请输入正确的手机号");

                if (address.Length == 0)
                    return AjaxResult("noaddress", "请输入详细地址");
                else if (address.Length > 75)
                    return AjaxResult("muchaddress", "详细地址不能超过75个字");

                OrderAfterServices.SendOrderAfterService(asId, shipCoId, shipCompanyInfo.Name, shipSN, regionId, consignee, mobile, address);
                return AjaxResult("success", orderAfterServiceInfo.Oid.ToString());
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult OrderProductAfterServiceList()
        {
            int recordId = WebHelper.GetQueryInt("recordId");

            OrderProductAfterServiceListModel model = new OrderProductAfterServiceListModel()
            {
                OrderAfterServiceList = OrderAfterServices.GetOrderProductAfterServiceList(recordId)
            };
            return View(model);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult CompleteOrderAfterService()
        {
            int asId = WebHelper.GetQueryInt("asId");
            ...
            if (orderAfterServiceInfo.State != (int)OrderAfterServiceState.Backed)
                return PromptView("商城还没有邮寄，不能完成");

            OrderAfterServices.CompleteOrderAfterService(asId);
