...
function buildCommonAttrSelectTable(result) {
    ...
    for (var i = 0; i < result.length; i++) {
        tableHtml += "<tr><td width='160px;' align='right'>";
...
function selectCommonAttrValue(obj) {
    ...
    if (attrValueObj.prop("checked") == true) {
        attrValueObj.parent().find("input[type=checkbox]").prop("checked", false);
        attrValueObj.parent().find("input[name=AttrIdList]").prop("checked", true);
        attrValueObj.prop("checked", true);
        if (isInput == "1") {//手动输入时
    ...
    if (productPageType == 1) {
        ...
        if (attrValueObj.prop("checked") == false) {
...
function buildSKUAttrSelectTable(result) {
    ...
    var tempInputAttrValueId = 0;
    for (var i = 0; i < result.length; i++) {
        tableHtml += "<tr name='attrTr'><td width='160px;' align='right'>" + result[i].name + "：</td><td><p>";
        for (var j = 0; j < result[i].attrValueList.length; j++) {
        ...
        tableHtml += "<label>手动输入<input type='text' name='inputValue' attrId='" + result[i].attrId + "' attrValueId='" + tempInputAttrValueId + "' class='input' size='20' onblur='selectSKUAttrValue()' value=''/><a href='javascript:void(0)' onclick='addSKUInputAttrValue(this," + result[i].attrId + "," + tempInputAttrValueId + ")' style='font-size:16px;'>+</a>&nbsp;&nbsp;</label>";
...
//添加sku输入属性值
function addSKUInputAttrValue(obj, attrId, attrValueId) {
    $(obj).parent().append("<label>手动输入<input type='text' name='inputValue' attrId='" + attrId + "' attrValueId='" + attrValueId + "' class='input' size='20' onblur='selectSKUAttrValue()' value=''/><a href='javascript:void(0)' onclick='addSKUInputAttrValue(this," + attrId + "," + attrValueId + ")' style='font-size:16px;'>+</a>&nbsp;<a href='javascript:void(0)' onclick='removeSKUInputAttrValue(this)'>-</a>&nbsp;</label>");
}

//删除sku输入属性值
function removeSKUInputAttrValue(obj) {
    $(obj).parent().remove();
    selectSKUAttrValue();
}
...
function selectSKUAttrValue() {
    ...
    for (var i = 0; i < attrTrList.length; i++) {
        ...
        //手动输入
        var inputAttrValueList = $(attrTrList[i]).find("input[name=inputValue]");
        for (var j = 0; j < inputAttrValueList.length; j++) {
            var inputAttrValueObj = $(inputAttrValueList[j]);
            var inputValue = inputAttrValueObj.val();
            if (inputValue != "") {
                var item = {
                    'attrId': inputAttrValueObj.attr("attrId"),
                    'attrValueId': inputAttrValueObj.attr("attrValueId"),
    ...
    for (var i = 1; i <= skuCount; i++) {
        skuListHtml += "<tr name='skuTr'><td width='160px' align='right'>SKU：</td><td><table style='border-bottom:1px solid #d1d1d1;'><tr>";
...
function setSelectedCategory(selectedCateId, selectedCateName) {
    ...
    if (productPageType == 0 || productPageType == 1 || productPageType == 3) {
        ...
        $.get("/admin/category/aandvjsonlist?cateId=" + selectedCateId + "&time=" + new Date(), function (data) {
            ...
            if (productPageType == 0 || productPageType == 1) {//添加，修改商品页面时
...
$(function () {
    ...
    if (productPageType != 2 && cateId > 0) {
        ...
        if (productPageType == 0 || productPageType == 1) {
            ...
            $.get("/admin/category/aandvjsonlist?cateId=" + cateId + "&time=" + new Date(), function (data) {
                ...
                if (productAttributeList != undefined && productAttributeList != null) {
                    for (var i = 0; i < productAttributeList.length; i++) {
                        $("#commonAttributeTable").find("input[name=AttrIdList][value=" + productAttributeList[i].attrId + "]").prop("checked", true);
                        $("#commonAttributeTable").find("input[name=AttrValueIdList][value=" + productAttributeList[i].attrValueId + "]").prop("checked", true);
                ...
                $(".inputattrvalue").on("focus", function () {
                    oldInputAttrValue = $(this).val();
                });

                $(".inputattrvalue").on("blur", function () {
            ...
            $.get("/admin/category/aandvjsonlist?cateId=" + cateId + "&time=" + new Date(), function (data) {
                ...
                if (productSKUItemList != undefined && productSKUItemList != null && productSKUItemList.length > 0) {
                    for (var i = 0; i < productSKUItemList.length; i++) {
                        $("#skuAttributeTable").find("input[name=attrValue][type=checkbox][value=" + productSKUItemList[i].attrValueId + "]").prop("checked", true);
        ...
        $(".shoppriceinput").focus(function () {
            ...
            shopPriceInputObj.prop("class", "selectedsortinput");
        });
        $(".shoppriceinput").blur(function () {
            ...
            if (shopPriceInputObj.val() == "") {
                ...
                if (!reg.test(shopPriceInputObj.val())) {
                    shopPriceInputObj.val(oldShopPriceInputValue).prop("class", "selectedsortinput");
                    alert("只能输入正数且最多两位小数！")
                    return;
                }
                else {
        ...
        $(".stockinput").focus(function () {
            ...
            stockInputObj.val("");
            stockInputObj.prop("class", "selectedsortinput");
        });
        $(".stockinput").blur(function () {
            ...
            if (stockInputObj.val() == "") {
                ...
                if (!reg.test(stockInputObj.val())) {
                    stockInputObj.val(oldStockInputValue).prop("class", "selectedsortinput");
                    alert("只能输入数字！")
                    return;
                }
                else {
            ...
            stockInputObj.prop("class", "unselectedsortinput");
        });
