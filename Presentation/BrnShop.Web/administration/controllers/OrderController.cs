...
namespace BrnShop.Web.Admin.Controllers
{
    ...
    public partial class OrderController : BaseAdminController
    {
        ...
        /// <returns></returns>
        public ActionResult OrderList(string osn, string accountName, string mobile, string consignee, int orderState = 0, int pageSize = 15, int pageNumber = 1)
        {
            //获取用户id
            int uid = Users.GetUidByAccountName(accountName);

            string condition = AdminOrders.GetOrderListCondition(osn, uid, mobile, consignee, orderState);
            ...
            ShopUtils.SetAdminRefererCookie(string.Format("{0}?pageNumber={1}&pageSize={2}&OSN={3}&AccountName={4}&Mobile={5}&Consignee={6}&OrderState={7}",
                                                          Url.Action("orderlist"),
                                                          pageModel.PageNumber,
                                                          pageModel.PageSize,
                                                          osn, accountName, mobile, consignee, orderState));
        ...
        public ActionResult OperateOrder(int oid = -1, int actionType = -1, string actionDes = "")
        {
            ...
            if (orderActionType == OrderActionType.Confirm)//确认订单
            {
                ...
                if (!string.IsNullOrWhiteSpace(orderInfo.Email))
                    Emails.SendOrderConfirmEmail(orderInfo.Email, orderInfo.OSN);
            }
            else if (orderActionType == OrderActionType.PreProduct)//备货
            {
                if (orderState != OrderState.Confirmed)
        ...
        /// <param name="gid">UID</param>
        /// <returns></returns>
        [HttpGet]
        public ActionResult UpdateOrderProductGoodsID(int recordid, string gid)
        {
            Orders.UpdateOrderProductGoodsID(recordid, gid);
            return AjaxResult("1", "", false);
        }
