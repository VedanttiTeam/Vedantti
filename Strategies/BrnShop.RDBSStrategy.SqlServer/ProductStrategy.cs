...
namespace BrnShop.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        /// <returns></returns>
        public DataTable AdminGetBrandList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} {2} FROM [{1}brands] ORDER BY [displayorder] DESC,[brandid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.BRANDS);

                else
                    commandText = string.Format("SELECT TOP {0} {3} FROM [{1}brands] WHERE {2} ORDER BY [displayorder] DESC,[brandid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition,
                                                TableFields.BRANDS);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} {3} FROM [{1}brands] WHERE [brandid] NOT IN (SELECT TOP {2} [brandid] FROM [{1}brands] ORDER BY [displayorder] DESC,[brandid] DESC) ORDER BY [displayorder] DESC,[brandid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                TableFields.BRANDS);
                else
                    commandText = string.Format("SELECT TOP {0} {4} FROM [{1}brands] WHERE [brandid] NOT IN (SELECT TOP {2} [brandid] FROM [{1}brands] WHERE {3} ORDER BY [displayorder] DESC,[brandid] DESC) AND {3} ORDER BY [displayorder] DESC,[brandid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                condition,
        ...
        {
            if (!string.IsNullOrWhiteSpace(brandName) && SecureHelper.IsSafeSqlString(brandName))
        ...
        public int GetBrandIdByName(string brandName)
        {
            ...
            string commandText = string.Format("SELECT [brandid] FROM [{0}brands] WHERE [name]=@name",
                                                RDBSHelper.RDBSTablePre);
        ...
        {
            DbParameter[] parms = {
                                        GenerateInParam("@brandid", SqlDbType.SmallInt, 2, brandId)
                                    };
            string vSql = @"SELECT [cateid],[displayorder],[name],[pricerange],[parentid],[layer],[haschild],[path] 
                            FROM[bsp_categories]
                            WHERE[cateid] IN(SELECT DISTINCT[cateid] FROM[bsp_products] WHERE[brandid] = @brandid AND[state] = 0) ORDER BY[displayorder] DESC";

            return RDBSHelper.ExecuteReader(CommandType.Text, vSql, parms);
        }
        #endregion
        ...
        /// <param name="cateId">分类id</param>
        /// <returns></returns>
        public DataTable AdminGetAttributeList(int cateId)
        {
            string commandText = string.Format("SELECT [temp1].[attrid],[temp1].[name],[temp1].[cateid],[temp1].[attrgroupid],[temp1].[showtype],[temp1].[isfilter],[temp1].[displayorder],[temp2].[name] AS [attrgroupname] FROM (SELECT {1} FROM [{0}attributes] WHERE [cateid]={2}) AS [temp1] LEFT JOIN [{0}attributegroups] AS [temp2] ON [temp1].[attrgroupid]=[temp2].[attrgroupid] ORDER BY [temp1].[displayorder] DESC,[temp1].[attrid] DESC",
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.ATTRIBUTES,
                                                cateId);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        ...
        public string AdminGetProductListCondition(string productName, int cateId, int brandId, int state)
        {
            ...
            if (!string.IsNullOrWhiteSpace(productName) && SecureHelper.IsSafeSqlString(productName, false))
        ...
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public DataTable AdminGetProductList(int pageSize, int pageNumber, string condition)
        {
            bool noCondition = string.IsNullOrWhiteSpace(condition);
            string commandText;
            if (pageNumber == 1)
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[pid],[temp1].[name],[temp1].[shopprice],[temp1].[marketprice],[temp1].[state],[temp1].[isbest],[temp1].[ishot],[temp1].[isnew],[temp1].[displayorder],[temp2].[number] FROM (SELECT TOP {0} [pid],[name],[shopprice],[marketprice],[state],[isbest],[ishot],[isnew],[displayorder] FROM [{1}products] WHERE [pid] NOT IN (SELECT TOP {2} [pid] FROM [{1}products] ORDER BY [displayorder] DESC,[pid] DESC) ORDER BY [displayorder] DESC,[pid] DESC) AS [temp1] LEFT JOIN [{1}productstocks] AS [temp2] ON [temp1].[pid]=[temp2].[pid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize);
                else
                    commandText = string.Format("SELECT [temp1].[pid],[temp1].[name],[temp1].[shopprice],[temp1].[marketprice],[temp1].[state],[temp1].[isbest],[temp1].[ishot],[temp1].[isnew],[temp1].[displayorder],[temp2].[number] FROM (SELECT TOP {0} [pid],[name],[shopprice],[marketprice],[state],[isbest],[ishot],[isnew],[displayorder] FROM [{1}products] WHERE [pid] NOT IN (SELECT TOP {2} [pid] FROM [{1}products] WHERE {3} ORDER BY [displayorder] DESC,[pid] DESC) AND {3} ORDER BY [displayorder] DESC,[pid] DESC) AS [temp1] LEFT JOIN [{1}productstocks] AS [temp2] ON [temp1].[pid]=[temp2].[pid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                condition);
        ...
        public int CreateProduct(ProductInfo productInfo)
        {
            DbParameter[] parms = {
                                     ...
                                     GenerateInParam("@weight",SqlDbType.Decimal,4,productInfo.Weight),
        ...
        public void UpdateProduct(ProductInfo productInfo)
        {
            ...
            string commandText = string.Format(@"UPDATE [{0}products] SET [psn]=@psn,[cateid]=@cateid,[brandid]=@brandid,[skugid]=@skugid,[name]=@name,[shopprice]=@shopprice,[marketprice]=@marketprice,[costprice]=@costprice,
                                                 ...
                                                 WHERE [pid]=@pid;
                                                 delete from [{0}productattributes] where [pid]=@pid and [attrid] not in(select [attrid] from [{0}attributes] where [cateid]=@cateid)  ",
                                                 RDBSHelper.RDBSTablePre);
        ...
        public IDataReader AdminGetProductById(int pid)
        {
            ...
            string commandText = string.Format(@"SELECT [p].[pid],[p].[psn],[p].[cateid],[p].[brandid],[p].[skugid],[p].[name],[p].[shopprice],[p].[marketprice],[p].[costprice],[p].[state],
                                                [p].[isbest],[p].[ishot],[p].[isnew],[p].[displayorder],[p].[weight],[p].[showimg],[p].[salecount],[p].[visitcount],[p].[reviewcount],isnull([ps].[number],0) [number],
                                                [p].[star1],[p].[star2],[p].[star3],[p].[star4],[p].[star5],[p].[addtime],[p].[description]
                                            FROM [{0}products] as [p] LEFT JOIN [{0}productstocks] AS [ps] ON [p].[pid]=[ps].[pid] WHERE [p].[pid]=@pid", RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        public IDataReader GetProductById(int pid)
        {
            ...
            string commandText = string.Format(@"SELECT [p].[pid],[p].[psn],[p].[cateid],[p].[brandid],[p].[skugid],[p].[name],[p].[shopprice],[p].[marketprice],[p].[costprice],[p].[state],
                                                [p].[isbest],[p].[ishot],[p].[isnew],[p].[displayorder],[p].[weight],[p].[showimg],[p].[salecount],[p].[visitcount],[p].[reviewcount],isnull([ps].[number],0) [number],
                                                [p].[star1],[p].[star2],[p].[star3],[p].[star4],[p].[star5],[p].[addtime],[p].[description]
                                            FROM [{0}products] as [p] LEFT JOIN [{0}productstocks] AS [ps] ON [p].[pid]=[ps].[pid] WHERE [p].[pid]=@pid and [p].[state]=0", RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        public IDataReader GetCategoryProductList(int pageSize, int pageNumber, int cateId, int brandId, int filterPrice, string[] catePriceRangeList, List<int> attrValueIdList, int onlyStock, int sortColumn, int sortDirection)
        {
            ...
            if (pageNumber == 1)
            {
                commandText.AppendFormat(@"SELECT TOP {1} [p].[pid],[p].[psn],[p].[cateid],[p].[brandid],[p].[skugid],[p].[name],[p].[shopprice],[p].[marketprice],[p].[costprice],[p].[state],
                                                [p].[isbest],[p].[ishot],[p].[isnew],[p].[displayorder],[p].[weight],[p].[showimg],[p].[salecount],[p].[visitcount],[p].[reviewcount],isnull([ps].[number],0) [number],
                                                [p].[star1],[p].[star2],[p].[star3],[p].[star4],[p].[star5],[p].[addtime],[p].[description] 
                                            FROM [{0}products] AS [p] LEFT JOIN [{0}productstocks] AS [ps] ON [p].[pid]=[ps].[pid]",
                                                RDBSHelper.RDBSTablePre, pageSize);
                ...
                if (filterPrice > 0 && catePriceRangeList != null && filterPrice <= catePriceRangeList.Length)
                {
                ...
                {
                    case 0:
                        commandText.Append("[p].[displayorder]");
                        break;
                    case 1:
                ...
                commandText.Append(",[p].[pid] DESC");
            }
            else
            {
                commandText.Append("SELECT [pid],[psn],[cateid],[brandid],[skugid],[name],[shopprice],[marketprice],[costprice],[state],[isbest],[ishot],[isnew],[displayorder],[weight],[showimg],[salecount],[visitcount],[reviewcount],[number],[star1],[star2],[star3],[star4],[star5],[addtime],[description] FROM");
                ...
                commandText.Append(",[p].[pid] DESC");
                commandText.AppendFormat(@") AS [rowid],[p].[pid],[p].[psn],[p].[cateid],[p].[brandid],[p].[skugid],[p].[name],[p].[shopprice],[p].[marketprice],[p].[costprice],[p].[state],
                                            [p].[isbest],[p].[ishot],[p].[isnew],[p].[displayorder],[p].[weight],[p].[showimg],[p].[salecount],[p].[visitcount],[p].[reviewcount],isnull([ps].[number],0) [number],
                                            [p].[star1],[p].[star2],[p].[star3],[p].[star4],[p].[star5],[p].[addtime],[p].[description] 
                                            FROM [{0}products] AS [p] LEFT JOIN [{0}productstocks] AS [ps] ON [p].[pid]=[ps].[pid]", RDBSHelper.RDBSTablePre);
        ...
        /// <param name="sortDirection">排序方向</param>
        /// <returns></returns>
        public IDataReader GetCategoryProductList(int pageSize, int pageNumber, int brandId, int cateId, int onlyStock, int sortColumn, int sortDirection)
        {
            StringBuilder commandText = new StringBuilder();
            ...
            if (pageNumber == 1)
            {
                commandText.AppendFormat(@"SELECT TOP {1} [p].[pid],[p].[psn],[p].[cateid],[p].[brandid],[p].[skugid],[p].[name],[p].[shopprice],[p].[marketprice],[p].[costprice],[p].[state],
[p].[isbest],[p].[ishot],[p].[isnew],[p].[displayorder],[p].[weight],[p].[showimg],[p].[salecount],[p].[visitcount],[p].[reviewcount],isnull([ps].[number],0) [number],
[p].[star1],[p].[star2],[p].[star3],[p].[star4],[p].[star5],[p].[addtime],[p].[description]  FROM [{0}products] AS [p] LEFT JOIN [{0}productstocks] AS [ps] ON [p].[pid]=[ps].[pid]", RDBSHelper.RDBSTablePre, pageSize);

                commandText.AppendFormat(" WHERE [p].[brandid]={0}", brandId);
            ...
            else
            {
                ...
                commandText.AppendFormat(@") AS [rowid],[p].[pid],[p].[psn],[p].[cateid],[p].[brandid],[p].[skugid],[p].[name],[p].[shopprice],[p].[marketprice],[p].[costprice],[p].[state],
[p].[isbest],[p].[ishot],[p].[isnew],[p].[displayorder],[p].[weight],[p].[showimg],[p].[salecount],[p].[visitcount],[p].[reviewcount],isnull([ps].[number],0) [number],
[p].[star1],[p].[star2],[p].[star3],[p].[star4],[p].[star5],[p].[addtime],[p].[description] FROM [{0}products] AS [p] LEFT JOIN [{0}productstocks] AS [ps] ON [p].[pid]=[ps].[pid]", RDBSHelper.RDBSTablePre);
                if (onlyStock == 1)
                    commandText.AppendFormat(" ", RDBSHelper.RDBSTablePre);
        ...
        /// <param name="onlyStock">是否只显示有货</param>
        /// <returns></returns>
        public int GetBrandProductCount(int brandId, int cateId = 0, int onlyStock = 0)
        {
            StringBuilder commandText = new StringBuilder(200);
            commandText.AppendFormat("SELECT COUNT([p].[pid]) FROM [{0}products] AS [p]", RDBSHelper.RDBSTablePre);
            commandText.AppendFormat(" WHERE [p].[brandid]={0}", brandId);
        ...
        {
            string commandText = string.Format("SELECT {1} FROM [{0}products] WHERE [pid] IN ({2}) ORDER BY CHARINDEX(','+CONVERT(varchar(10),[pid])+',',','+'{2}'+',')",
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.PART_PRODUCTS,
                                                pidList);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        ...
        public IDataReader GetProductAttributeByPidAndAttrId(int pid, int attrId)
        {
            ...
            string commandText = string.Format("SELECT {1} FROM [{0}productattributes] WHERE [pid]=@pid AND [attrid]=@attrid",
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.PRODUCT_ATTRIBUTES);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        /// 创建商品图片
        /// </summary>
        public void CreateProductImage(ProductImageInfo productImageInfo)
        {
            DbParameter[] parms =  {
            ...
            if (productImageInfo.IsMain == 1)
                commandText.AppendFormat("UPDATE [{0}productimages] SET [ismain]=0 WHERE [pid]=@pid AND [ismain]=1;", RDBSHelper.RDBSTablePre);
            commandText.AppendFormat("INSERT INTO [{0}productimages]([pid],[showimg],[ismain],[displayorder]) VALUES(@pid,@showimg,@ismain,@displayorder)", RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText.ToString(), parms);
        }
        ...
        public void DeleteProductImageById(int pImgId)
        {
            DbParameter[] parms =  {
                                     GenerateInParam("@pimgid", SqlDbType.Int, 4, pImgId)
                                   };
        ...
        /// <param name="pid">商品id</param>
        /// <param name="pImgId">商品图片id</param>
        public void SetProductMainImage(int pid, int pImgId)
        {
            DbParameter[] parms =  {
                                     GenerateInParam("@pid", SqlDbType.Int, 4, pid),
                                     GenerateInParam("@pimgid", SqlDbType.Int, 4, pImgId)
                                   };
            string commandText = string.Format("UPDATE [{0}productimages] SET [ismain]=0 WHERE [pid]=@pid AND [ismain]=1;UPDATE [{0}productimages] SET [ismain]=1 WHERE [pimgid]=@pimgid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public void ChangeProductImageDisplayOrder(int pImgId, int displayOrder)
        {
            DbParameter[] parms =  {
                                     GenerateInParam("@displayorder", SqlDbType.Int, 4, displayOrder),
                                     GenerateInParam("@pimgid", SqlDbType.Int, 4, pImgId)
        ...
        public IDataReader GetTimeProductList(int type)
        {
            string commandText;
            if (type == 0)
                commandText = string.Format("SELECT {1} FROM [{0}timeproducts] WHERE [onsalestate]=1 AND [onsaletime]<='{2}' AND ([outsalestate]=0 OR ([outsalestate]>0 AND [outsaletime]>'{2}'))", RDBSHelper.RDBSTablePre, TableFields.TIMEPRODUCTS, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
            else
                commandText = string.Format("SELECT {1} FROM [{0}timeproducts] WHERE [outsalestate]=1 AND [outsaletime]<='{2}'", RDBSHelper.RDBSTablePre, TableFields.TIMEPRODUCTS, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        public DataTable GetTimeProductList(int pageSize, int pageNumber, string productName)
        {
            ...
            if (pageNumber == 1)
            {
                if (string.IsNullOrWhiteSpace(productName) || !SecureHelper.IsSafeSqlString(productName))
        ...
        public DataTable GetProductConsultTypeList()
        {
            string commandText = string.Format("SELECT {1} FROM [{0}productconsulttypes] ORDER BY [displayorder] DESC",
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.PRODUCT_CONSULTTYPES);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
		...
		/// </summary>
		/// <returns></returns>
		public DataTable GetProductSubscribelist()
        {
            string commandText = string.Format("select FID, fvchrEmail, fvchrIP, fdtmCreateDate, fintDisabled from {0}subscribe order by fdtmCreateDate desc;", RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        public void DeleteProductSubscribeById(string SubscribeId)
        {
            DbParameter[] parms = {
                                    GenerateInParam("@SubscribeId", SqlDbType.UniqueIdentifier, 1, new Guid(SubscribeId))
                                  };
            string commandText = string.Format("DELETE FROM [{0}subscribe] WHERE [FID]=@SubscribeId;", RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        #endregion
        ...
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public IDataReader AdminGetProductConsultList(int pageSize, int pageNumber, string condition)
        {
            bool noCondition = string.IsNullOrWhiteSpace(condition);
            string commandText;
            if (pageNumber == 1)
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} {1} FROM [{2}productconsults] ORDER BY [consultid] DESC",
                                                pageSize,
                                                TableFields.PRODUCT_CONSULTS,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT TOP {0} {1} FROM [{2}productconsults] WHERE {3} ORDER BY [consultid] DESC",
                                                pageSize,
                                                TableFields.PRODUCT_CONSULTS,
                                                RDBSHelper.RDBSTablePre,
                                                condition);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT {0} FROM (SELECT TOP {2} ROW_NUMBER() OVER (ORDER BY [consultid] DESC) AS [rowid],{0} FROM [{1}productconsults]) AS [temp] WHERE [temp].[rowid] BETWEEN {3} AND {2}",
                                                TableFields.PRODUCT_CONSULTS,
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1);
                else
                    commandText = string.Format("SELECT {0} FROM (SELECT TOP {2} ROW_NUMBER() OVER (ORDER BY [consultid] DESC) AS [rowid],{0} FROM [{1}productconsults] WHERE {4}) AS [temp] WHERE [temp].[rowid] BETWEEN {3} AND {2}",
                                                TableFields.PRODUCT_CONSULTS,
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1,
        ...
        public string AdminGetProductConsultListCondition(int consultTypeId, int pid, int uid, string consultMessage, string consultStartTime, string consultEndTime)
        {
            ...
            if (!string.IsNullOrWhiteSpace(consultMessage) && SecureHelper.IsSafeSqlString(consultMessage))
        ...
        /// <param name="condition">条件</param>
        /// <returns></returns>
        public DataTable AdminGetProductReviewList(int pageSize, int pageNumber, string condition)
        {
            bool noCondition = string.IsNullOrWhiteSpace(condition);
            string commandText;
            if (pageNumber == 1)
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[reviewid],[temp1].[pid],[temp1].[uid],[temp1].[oprid],[temp1].[oid],[temp1].[parentid],[temp1].[state],[temp1].[star],[temp1].[quality],[temp1].[message],[temp1].[reviewtime],[temp1].[pname],[temp1].[pshowimg],[temp1].[buytime],[temp1].[ip],[temp2].[nickname] FROM (SELECT TOP {0} {2} FROM [{1}productreviews] ORDER BY [reviewid] DESC) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                TableFields.PRODUCT_REVIEWS);
                else
                    commandText = string.Format("SELECT [temp1].[reviewid],[temp1].[pid],[temp1].[uid],[temp1].[oprid],[temp1].[oid],[temp1].[parentid],[temp1].[state],[temp1].[star],[temp1].[quality],[temp1].[message],[temp1].[reviewtime],[temp1].[pname],[temp1].[pshowimg],[temp1].[buytime],[temp1].[ip],[temp2].[nickname] FROM (SELECT TOP {0} {3} FROM [{1}productreviews] WHERE {2} ORDER BY [reviewid] DESC) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition,
                                                TableFields.PRODUCT_REVIEWS);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[reviewid],[temp1].[pid],[temp1].[uid],[temp1].[oprid],[temp1].[oid],[temp1].[parentid],[temp1].[state],[temp1].[star],[temp1].[quality],[temp1].[message],[temp1].[reviewtime],[temp1].[pname],[temp1].[pshowimg],[temp1].[buytime],[temp1].[ip],[temp2].[nickname] FROM (SELECT TOP {2} ROW_NUMBER() OVER (ORDER BY [reviewid] DESC) AS [rowid],{0} FROM [{1}productreviews]) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] WHERE [temp1].[rowid] BETWEEN {3} AND {2}",
                                                TableFields.PRODUCT_REVIEWS,
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1);
                else
                    commandText = string.Format("SELECT [temp1].[reviewid],[temp1].[pid],[temp1].[uid],[temp1].[oprid],[temp1].[oid],[temp1].[parentid],[temp1].[state],[temp1].[star],[temp1].[quality],[temp1].[message],[temp1].[reviewtime],[temp1].[pname],[temp1].[pshowimg],[temp1].[buytime],[temp1].[ip],[temp2].[nickname] FROM (SELECT TOP {2} ROW_NUMBER() OVER (ORDER BY [reviewid] DESC) AS [rowid],{0} FROM [{1}productreviews] WHERE {4}) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] WHERE [temp1].[rowid] BETWEEN {3} AND {2}",
                                                TableFields.PRODUCT_REVIEWS,
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1,
        ...
        public string AdminGetProductReviewListCondition(int pid, string message, string startTime, string endTime)
        {
            ...
            if (!string.IsNullOrWhiteSpace(message) && SecureHelper.IsSafeSqlString(message))
        ...
        /// <param name="word">搜索词</param>
        /// <returns></returns>
        public DataTable GetSearchWordStatList(int pageSize, int pageNumber, string word)
        {
            string commandText;
            if (pageNumber == 1)
            {
                if (string.IsNullOrWhiteSpace(word) || !SecureHelper.IsSafeSqlString(word))
                    commandText = string.Format("SELECT TOP {0} [word],SUM([times]) AS [totaltimes],MAX([updatetime]) AS [lastsearchtime] FROM [{1}searchhistories] GROUP BY [word] ORDER BY [totaltimes] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT TOP {0} [word],SUM([times]) AS [totaltimes],MAX([updatetime]) AS [lastsearchtime] FROM [{1}searchhistories] WHERE [word] LIKE '{2}%' GROUP BY [word] ORDER BY [totaltimes] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                word);
            }
            ...
            {
                if (string.IsNullOrWhiteSpace(word) || !SecureHelper.IsSafeSqlString(word))
                    commandText = string.Format("SELECT [word],[totaltimes],[lastsearchtime] FROM (SELECT TOP {1} ROW_NUMBER() OVER (ORDER BY [totaltimes] DESC) AS [rowid],[word],SUM([times]) AS [totaltimes],MAX([updatetime]) AS [lastsearchtime] FROM [{0}searchhistories] GROUP BY [word] ORDER BY [totaltimes] DESC) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {1}",
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1);
                else
                    commandText = string.Format("SELECT [word],[totaltimes],[lastsearchtime] FROM (SELECT TOP {1} ROW_NUMBER() OVER (ORDER BY [totaltimes] DESC) AS [rowid],[word],SUM([times]) AS [totaltimes],MAX([updatetime]) AS [lastsearchtime] FROM [{0}searchhistories] WHERE [word] LIKE '{3}%' GROUP BY [word] ORDER BY [totaltimes] DESC) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {1}",
                                                RDBSHelper.RDBSTablePre,
                                                pageNumber * pageSize,
                                                (pageNumber - 1) * pageSize + 1,
                                                word);
