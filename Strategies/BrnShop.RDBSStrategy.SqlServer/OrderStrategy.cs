...
namespace BrnShop.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable GetOrderList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[oid],[temp1].[osn],[temp1].[uid],[temp1].[orderstate],[temp1].[orderamount],[temp1].[surplusmoney],[temp1].[isreview],[temp1].[addtime],[temp1].[regionid],[temp1].[consignee],[temp1].[mobile],[temp1].[phone],[temp1].[email],[temp1].[zipcode],[temp1].[address],[temp1].[besttime],[temp2].[username],[temp3].[provincename],[temp3].[cityname],[temp3].[name] AS [countyname] FROM (SELECT TOP {0} [oid],[osn],[uid],[orderstate],[orderamount],[surplusmoney],[isreview],[addtime],[regionid],[consignee],[mobile],[phone],[email],[zipcode],[address],[besttime] FROM [{1}orders] ORDER BY [oid] DESC) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}regions] AS [temp3] ON [temp1].[regionid]=[temp3].[regionid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[oid],[temp1].[osn],[temp1].[uid],[temp1].[orderstate],[temp1].[orderamount],[temp1].[surplusmoney],[temp1].[isreview],[temp1].[addtime],[temp1].[regionid],[temp1].[consignee],[temp1].[mobile],[temp1].[phone],[temp1].[email],[temp1].[zipcode],[temp1].[address],[temp1].[besttime],[temp2].[username],[temp3].[provincename],[temp3].[cityname],[temp3].[name] AS [countyname] FROM (SELECT [oid],[osn],[uid],[orderstate],[orderamount],[surplusmoney],[isreview],[addtime],[regionid],[consignee],[mobile],[phone],[email],[zipcode],[address],[besttime] FROM (SELECT TOP {0} ROW_NUMBER() OVER (ORDER BY [oid] DESC) AS [rowid],[oid],[osn],[uid],[orderstate],[orderamount],[surplusmoney],[isreview],[addtime],[regionid],[consignee],[mobile],[phone],[email],[zipcode],[address],[besttime] FROM [{1}orders]) AS [temp] WHERE [temp].[rowid] BETWEEN {2} AND {3}) AS [temp1] LEFT JOIN [{1}users] AS [temp2] ON [temp1].[uid]=[temp2].[uid] LEFT JOIN [{1}regions] AS [temp3] ON [temp1].[regionid]=[temp3].[regionid]",
                                                pageSize * pageNumber,
                                                RDBSHelper.RDBSTablePre,
        ...
        /// <param name="orderState">订单状态</param>
        /// <returns></returns>
        public string GetOrderListCondition(string osn, int uid, string mobile, string consignee, int orderState)
        {
            StringBuilder condition = new StringBuilder();
            ...
            if (!string.IsNullOrWhiteSpace(mobile))
                condition.AppendFormat(" AND [mobile] = '{0}' ", mobile);
        ...
        /// <returns></returns>
        public bool HasBuyGoods(int uid)
        {
            DbParameter[] parms = {
                                     GenerateInParam("@uid", SqlDbType.Int,30,uid)
                                   };
            string commandText = string.Format("select top 1 oid from [{0}orders] where [uid]=@uid and orderstate=120", RDBSHelper.RDBSTablePre);
            object vResult = RDBSHelper.ExecuteScalar(CommandType.Text, commandText, parms);
            return vResult != null;
        }
        ...
        /// <param name="gid">商品货号</param>
        /// <returns></returns>
        public void UpdateOrderProductGoodsID(int recordid,string gid)
        {
            DbParameter[] parms = {
                                     GenerateInParam("@recordid", SqlDbType.Int,4,recordid),
                                     GenerateInParam("@gid", SqlDbType.VarChar,30,gid)
                                   };
            string commandText = string.Format("UPDATE [{0}orderproducts] SET [gid]=@gid WHERE [recordid]=@recordid", RDBSHelper.RDBSTablePre);
        ...
        /// <param name="orderState">订单状态</param>
        /// <returns></returns>
        public DataTable GetSaleProductList(int pageSize, int pageNumber, string startTime, string endTime, int orderState)
        {
            string condition = GetSaleProductListCondition(startTime, endTime, orderState);
            string commandText;
            if (pageNumber == 1)
            {
        ...
        /// <param name="orderState">订单状态</param>
        /// <returns></returns>
        public int GetSaleProductCount(string startTime, string endTime, int orderState)
        {
            string condition = GetSaleProductListCondition(startTime, endTime, orderState);
        ...
        /// <param name="orderState">订单状态</param>
        /// <returns></returns>
        private string GetSaleProductListCondition(string startTime, string endTime, int orderState)
        {
            StringBuilder condition = new StringBuilder();
        ...
        /// <param name="endTime">结束时间</param>
        /// <returns></returns>
        public DataTable GetOrderStat(int statType, string startTime, string endTime)
        {
            StringBuilder condition = new StringBuilder();
        ...
        public void ApplyOrderAfterService(OrderAfterServiceState state, int uid, int oid, int recordId, int pid, int cateId, int brandId, string pname, string pshowImg, int count, decimal money, int type, string applyReason, DateTime applyTime)
        {
            DbParameter[] parms = {
        ...
        public DataTable GetOrderProductAfterServiceList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} [asid],[state],[uid],[oid],[recordid],[pid],[pname],[pshowimg],[count],[money],[type],[applytime],[checkresult],[checktime],[regionid],[sendtime],[receivetime],[backtime] FROM [{1}orderafterservices] ORDER BY [asid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT TOP {0} [asid],[state],[uid],[oid],[recordid],[pid],[pname],[pshowimg],[count],[money],[type],[applytime],[checkresult],[checktime],[regionid],[sendtime],[receivetime],[backtime] FROM [{1}orderafterservices] WHERE {2} ORDER BY [asid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition);
            }
        ...
        /// <returns></returns>
        public string GetOrderProductAfterServiceListCondition(int uid, int oid, int state, int type, string applyStartTime, string applyEndTime)
        {
            StringBuilder condition = new StringBuilder();
        ...
        public void ApplyRefund(OrderRefundInfo orderRefundInfo)
        {
            ...
            string commandText = string.Format("INSERT INTO [{0}orderrefunds]([state],[oid],[osn],[uid],[asid],[paysystemname],[payfriendname],[paysn],[paymoney],[refundmoney],[applytime],[refundsn],[refundtime]) VALUES(@state,@oid,@osn,@uid,@asid,@paysystemname,@payfriendname,@paysn,@paymoney,@refundmoney,@applytime,@refundsn,@refundtime)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
