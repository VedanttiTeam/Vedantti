...
namespace BrnShop.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public DataTable AdminGetUserList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[admingid],[{1}users].[nickname],[{1}users].[paycredits],[{1}users].[rankcredits],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}admingroups].[title] AS [atitle] FROM [{1}users] LEFT JOIN [{1}userdetails] ON [{1}userdetails].[uid] = [{1}users].[uid]  LEFT JOIN [{1}userranks] ON [{1}userranks].[userrid]=[{1}users].[userrid]  LEFT JOIN [{1}admingroups] ON [{1}admingroups].[admingid]=[{1}users].[admingid] ORDER BY [{1}users].[uid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[admingid],[{1}users].[nickname],[{1}users].[paycredits],[{1}users].[rankcredits],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}admingroups].[title] AS [atitle] FROM [{1}users] LEFT JOIN [{1}userdetails] ON [{1}userdetails].[uid] = [{1}users].[uid]  LEFT JOIN [{1}userranks] ON [{1}userranks].[userrid]=[{1}users].[userrid]  LEFT JOIN [{1}admingroups] ON [{1}admingroups].[admingid]=[{1}users].[admingid] WHERE {2} ORDER BY [{1}users].[uid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);
            }
            else
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[admingid],[{1}users].[nickname],[{1}users].[paycredits],[{1}users].[rankcredits],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}admingroups].[title] AS [atitle] FROM [{1}users],[{1}userdetails],[{1}userranks],[{1}admingroups]  WHERE [{1}userdetails].[uid] = [{1}users].[uid] AND  [{1}userranks].[userrid]=[{1}users].[userrid] AND  [{1}admingroups].[admingid]=[{1}users].[admingid] AND [{1}users].[uid] < (SELECT min([uid])  FROM (SELECT TOP {2} [uid] FROM [{1}users] ORDER BY [uid] DESC) AS temp ) ORDER BY [{1}users].[uid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize);
                else
                    commandText = string.Format("SELECT TOP {0} [{1}users].[uid],[{1}users].[username],[{1}users].[email],[{1}users].[mobile],[{1}users].[userrid],[{1}users].[admingid],[{1}users].[nickname],[{1}users].[paycredits],[{1}users].[rankcredits],[{1}userdetails].[lastvisittime],[{1}userdetails].[lastvisitip],[{1}userdetails].[registertime],[{1}userdetails].[gender],[{1}userdetails].[realname],[{1}userranks].[title] AS [utitle],[{1}admingroups].[title] AS [atitle] FROM [{1}users],[{1}userdetails],[{1}userranks],[{1}admingroups]  WHERE [{1}userdetails].[uid] = [{1}users].[uid] AND  [{1}userranks].[userrid]=[{1}users].[userrid] AND  [{1}admingroups].[admingid]=[{1}users].[admingid] AND [{1}users].[uid] < (SELECT min([uid])  FROM (SELECT TOP {2} [uid] FROM [{1}users] WHERE {3} ORDER BY [uid] DESC) AS temp ) AND {3} ORDER BY [{1}users].[uid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                condition);
        ...
        /// </summary>
        /// <param name="uid">用户id.</param>
        public void UpdateUserVerityEmailByUid(int uid)
        {
            DbParameter[] parms = {
                                       GenerateInParam("@uid",SqlDbType.Int,4, uid)
                                   };
            RDBSHelper.ExecuteNonQuery(CommandType.Text,
                                       string.Format("update {0}users set verifyemail=1 where uid=@uid", RDBSHelper.RDBSTablePre),
                                       parms);
        }
