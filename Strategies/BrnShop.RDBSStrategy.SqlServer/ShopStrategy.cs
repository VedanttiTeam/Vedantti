    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public void CreateNews(NewsInfo newsInfo)
        {
            DbParameter[] parms = {
                                        ...
                                        GenerateInParam("@image", SqlDbType.NVarChar,200,newsInfo.Image),
                                        GenerateInParam("@url", SqlDbType.NVarChar,200,newsInfo.Url),
                                        GenerateInParam("@body", SqlDbType.NText, 0, newsInfo.Body)
                                    };
            string commandText = string.Format("INSERT INTO [{0}news]([newstypeid],[isshow],[istop],[ishome],[displayorder],[addtime],[title],[image],[url],[body]) VALUES(@newstypeid,@isshow,@istop,@ishome,@displayorder,@addtime,@title,@image,@url,@body)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public void UpdateNews(NewsInfo newsInfo)
        {
            ...
            string commandText = string.Format("UPDATE [{0}news] SET [newstypeid]=@newstypeid,[isshow]=@isshow,[istop]=@istop,[ishome]=@ishome,[displayorder]=@displayorder,[addtime]=@addtime,[title]=@title,[image]=@image,[url]=@url,[body]=@body WHERE [newsid]=@newsid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public DataTable GetHomeNewsList(int newsTypeId, int count)
        {
            string commandText;
            if (newsTypeId == 0)
                commandText = string.Format("SELECT [temp2].[name],[temp1].[newsid],[temp1].[newstypeid],[temp1].[addtime],[temp1].[title],[temp1].[image],[temp1].[url] FROM (SELECT TOP {1} [newsid],[newstypeid],[addtime],[title],[image],[url] FROM [{0}news] WHERE [isshow]=1 AND [ishome]=1 ORDER BY [displayorder] DESC) AS [temp1] LEFT JOIN [{0}newstypes] AS [temp2] ON [temp1].[newstypeid]=[temp2].[newstypeid]", RDBSHelper.RDBSTablePre, count);
            else
                commandText = string.Format("SELECT [temp2].[name],[temp1].[newsid],[temp1].[newstypeid],[temp1].[addtime],[temp1].[title],[temp1].[image],[temp1].[url] FROM (SELECT TOP {2} [newsid],[newstypeid],[addtime],[title],[image],[url] FROM [{0}news] WHERE [newstypeid]={1} AND [isshow]=1 AND [ishome]=1 ORDER BY [displayorder] DESC) AS [temp1] LEFT JOIN [{0}newstypes] AS [temp2] ON [temp1].[newstypeid]=[temp2].[newstypeid]", RDBSHelper.RDBSTablePre, newsTypeId, count);
            return RDBSHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        /// </summary>
        /// <returns></returns>
        public IDataReader GetShipCompanyList()
        {
            string commandText = string.Format("SELECT {0} FROM [{1}shipcompanies] ORDER BY [displayorder] DESC",
                                                TableFields.SHIPCOMPANIES,
                                                RDBSHelper.RDBSTablePre);
            return RDBSHelper.ExecuteReader(CommandType.Text, commandText);
        }
        ...
        public void UpdateShipCompany(ShipCompanyInfo shipCompanyInfo)
        {
            DbParameter[] parms = {
                                    GenerateInParam("@name",SqlDbType.NChar, 30, shipCompanyInfo.Name),
                                    GenerateInParam("@displayorder",SqlDbType.Int,4,shipCompanyInfo.DisplayOrder),
                                    GenerateInParam("@shipcoid", SqlDbType.Int, 4, shipCompanyInfo.ShipCoId)
                                   };
        ...
        {
            DbParameter[] parms = {
                                    GenerateInParam("@shipcoid", SqlDbType.Int, 4, shipCoId)
                                   };
        ...
        /// </summary>
        /// <param name="creditLogInfo">积分日志信息</param>
        public void SendPayCredits(CreditLogInfo creditLogInfo)
        {
            DbParameter[] parms = {
                                       GenerateInParam("@uid",SqlDbType.Int,4,creditLogInfo.Uid),
                                       GenerateInParam("@paycredits",SqlDbType.Int,4,creditLogInfo.PayCredits),
            ...
            RDBSHelper.ExecuteNonQuery(CommandType.StoredProcedure,
                                       string.Format("{0}sendpaycredits", RDBSHelper.RDBSTablePre),
                                       parms);
        }
        ...
        /// <param name="ip">IP</param>
        public bool SubscribeEmail(string email, string ip)
        {
            DbParameter[] parms = {
                                    GenerateInParam("@fvchrEmail",SqlDbType.VarChar,300, email),
                                    GenerateInParam("@fvchrIP",SqlDbType.VarChar,300, ip),
                                   };
            RDBSHelper.ExecuteNonQuery(CommandType.StoredProcedure, string.Format("{0}addsubscribe", RDBSHelper.RDBSTablePre), parms);
            return true;
        }
        /// <summary>
        /// 退订阅邮件
        /// </summary>
        /// <param name="email">电子邮件</param>
        public bool DelSubscribeEmail(string email)
        {
            DbParameter[] parms = {
                                    GenerateInParam("@fvchrEmail",SqlDbType.VarChar,300, email)
                                   };
            RDBSHelper.ExecuteNonQuery(CommandType.StoredProcedure, string.Format("{0}delsubscribe", RDBSHelper.RDBSTablePre), parms);
            return true;
        }
        #endregion  订阅
    }
