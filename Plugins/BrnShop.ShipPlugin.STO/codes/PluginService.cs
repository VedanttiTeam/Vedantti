...
namespace BrnShop.ShipPlugin.STO
{
    /// <summary>
    /// 插件服务类
    /// </summary>
    public class PluginService : IShipPlugin
    {
        ...
        public decimal GetShipFee(decimal totalWeight, decimal productAmount, List<OrderProductInfo> orderProductList, DateTime buyTime, int provinceId, int cityId, int countyId, PartUserInfo partUserInfo)
        {
            foreach (ShipRuleInfo shipRuleInfo in PluginUtils.GetShipRuleList())
            {
                if (shipRuleInfo.RegionId == 0 || shipRuleInfo.RegionId == provinceId || shipRuleInfo.RegionId == cityId || shipRuleInfo.RegionId == countyId)
                    ...
                    if (shipRuleInfo.Type == 0)
                    {
                        ...
                        else
                        {
                            if (totalWeight % 1000 == 0)
                                return shipRuleInfo.ExtCode1 + shipRuleInfo.ExtCode2 * ((totalWeight - 1000) / 1000);
                            else
                                return shipRuleInfo.ExtCode1 + shipRuleInfo.ExtCode2 * ((totalWeight - 1000) / 1000 + 1);
                        }
                    }
